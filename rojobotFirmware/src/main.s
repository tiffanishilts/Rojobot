...
PORT_INTACK         = 0x80001804        # (o) Bot Int Ack -- modified
PORT_BOTUPDT        = 0x80001801        # (i) Bot Update port (Poll) -- code using this is commented out so I'll assume I don't need this
PORT_PBEN           = 0x80001808        
...
_start:
                ...
                li s10, 0  // initialize bactrack orientation
                li s11, 0  // initialize hold state variable
...
next_step_auto: 
		...
		//follow black line when you  turn on the SW[15].
                ...
                li t1, 5
                beq t1, s1, ENDTURN   // if in state five, go to end turn

                li t1, 6
                beq t1, s1, BLOCKED   // if in state six, go to blocked

                li t1, 7
                beq t1, s1, CHECKBACKTRACK   // if in state seven, go to check backtrack

                li t1, 8
                beq t1, s1, HOLD   // if in state eight, go to hold
                ...
                j set_back_track   // set the backtrack orientation
                ...
                li t1, 4
                beq t1, s4, ENDTURN   // check turn counter, branch if 45 degrees

                addi s4, s4, 1   // increment turn counter
                ...
                andi t1, t1, 0x00000007   // mask for orientation bits

                bne s10, t1, HOLD   // if orientation is not backtrack orientation, go to hold state, else keep turning

                li t2, 0x00           // load value to turn left and right motor off
                sb t2, 0(s2)          // STOP

                li s1, 3   // load turning state
                ...
                li t1, 5 
                beq s11, t1, INIT   // check hold state counter, branch to init if done

                addi s11, s11, 1   // increment hold state counter
